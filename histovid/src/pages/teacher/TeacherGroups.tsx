...
export default function TeacherGroups() {
  ...
  const handleCreate = async () => {
    ...
    setSaving(false)
    setShowModal(false)
    setForm({ name: '', description: '', grade: '', school_year: form.school_year })
    loadGroups()
  }
        ...
        <div className="fixed inset-0 bg-ink-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-parchment-50 rounded-sm shadow-raised w-full max-w-md animate-slide-up border border-parchment-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.grade} onChange={e => setForm(f => ({ ...f, grade: e.target.value }))} placeholder="ej. 2° Secundaria" className="w-full border border-parchment-300 rounded px-3 py-2 font-body text-ink-800 bg-white focus:outline-none focus:border-gold-400" />
                </div>
                <div>
            ...
            <div className="flex gap-3 p-6 pt-0">
              ...
              <button onClick={handleCreate} disabled={!form.name.trim() || saving} className="flex-1 bg-crimson-500 text-parchment-50 py-2.5 rounded-sm font-body font-medium hover:bg-crimson-600 disabled:opacity-40 transition-colors">
